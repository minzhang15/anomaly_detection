...
def val(cfg, model=None):
    ...
    if model:  # This is for testing during training.
        ...
        generator = UNet(input_channels=12, output_channel=3).to(device).eval()
        generator.load_state_dict(torch.load('weights/' + cfg.trained_model, map_location=torch.device('cpu'))['net_g'])
    ...
    with torch.no_grad():
        for i, folder in enumerate(video_folders):
            ...
            for j, clip in enumerate(dataset):
                ...
                device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                input_frames = torch.from_numpy(input_np).unsqueeze(0).to(device)
                target_frame = torch.from_numpy(target_np).unsqueeze(0).to(device)
